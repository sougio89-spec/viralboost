...
// ── SITEMAP.XML — déclaré AVANT express.static pour éviter qu'un fichier physique prenne la priorité ──
app.get('/sitemap.xml', (req, res) => {
  const BASE = 'https://viralboost-ad.onrender.com';
  const now = new Date().toISOString().split('T')[0];

  const staticRoutes = [
    { url: '/',         priority: '1.0', changefreq: 'daily'  },
    { url: '/#feed',    priority: '0.9', changefreq: 'hourly' },
    { url: '/#vitrine', priority: '0.8', changefreq: 'daily'  },
    { url: '/#groupes', priority: '0.8', changefreq: 'daily'  },
    { url: '/#collabs', priority: '0.7', changefreq: 'weekly' },
    { url: '/#pricing', priority: '0.7', changefreq: 'weekly' },
  ];

  const projectRoutes = db.projects.slice(0, 100).map(p => ({
    url: `/#project-${p.id}`,
    priority: '0.6',
    changefreq: 'weekly',
    lastmod: p.createdAt ? p.createdAt.split('T')[0] : now,
  }));
...
app.get('/robots.txt', (req, res) => {
  res.type('text/plain');
  res.send('User-agent: *\nAllow: /\nSitemap: https://viralboost-ad.onrender.com/sitemap.xml');
});
