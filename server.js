...
// ════════════════════════════════════════
// ── REDIRECTIONS CANONIQUES SEO
// ── But : Google ne doit voir qu'UNE seule URL : https://viralboost.fr/
// ── 1) www.viralboost.fr  → viralboost.fr  (301)
// ── 2) http://            → https://       (301) — si pas derrière un proxy SSL
// ── 3) /chemin            → /chemin/       (301) pour les routes SPA sans slash
// ════════════════════════════════════════
app.use((req, res, next) => {
  ...
  if (host.startsWith('www.')) {
    const canonical = `https://viralboost.fr${req.originalUrl}`;
    return res.redirect(301, canonical);
  }

  // 2) Forcer HTTPS (uniquement si on n'est pas déjà en HTTPS — utile si Render passe le proto)
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
  ...
  <url>
    ...
    <xhtml:link rel="alternate" hreflang="en"        href="https://viralboost.fr/"/>
    <xhtml:link rel="alternate" hreflang="fr"        href="https://viralboost.fr/fr/"/>
    <xhtml:link rel="alternate" hreflang="x-default" href="https://viralboost.fr/"/>
  </url>
  <url>
    <loc>https://viralboost.fr/fr/</loc>
    <lastmod>${now}</lastmod>
    <changefreq>weekly</changefreq>
...
// ── ROBOTS.TXT — servi statiquement, jamais redirigé ──
app.get('/robots.txt', (req, res) => {
...
// ── SITEMAP.XML — une seule URL canonique ──
app.get('/sitemap.xml', (req, res) => {
...
// ── FALLBACK SPA ──
// IMPORTANT : on NE redirige PLUS les URLs inconnues.
// Googlebot ne voit qu'une seule URL canonique (/) grâce au sitemap + canonical meta.
// Toute route inconnue reçoit index.html avec un 200 (comportement SPA standard).
// Le <link rel="canonical" href="https://viralboost.fr/"> dans index.html
// dit explicitement à Google qu'il n'y a qu'une seule page à indexer.
const STATIC_EXTENSIONS = /\.(js|css|png|jpg|jpeg|gif|ico|svg|webp|woff|woff2|ttf|eot|json|xml|txt|map)$/i;

// ── ROUTES CONNUES DE LA SPA (Google ne doit PAS les indexer séparément)
// Ces routes répondent en 200 avec index.html + header canonical
// pour que Googlebot comprenne que tout pointe vers https://viralboost.fr/
const SPA_ROUTES = ['/fr', '/fr/'];

app.get('*', (req, res) => {
  // Fichiers statiques absents → 404 propre (pas de redirect)
  if (STATIC_EXTENSIONS.test(req.path)) {
    return res.status(404).send('Not found');
  }

  // Ajouter un header Link canonical sur toutes les réponses SPA
  // Indique aux crawlers que l'URL canonique est toujours la racine
  res.setHeader('Link', '<https://viralboost.fr/>; rel="canonical"');

  // SPA : toujours servir index.html avec 200, jamais de redirect
  // Le <link rel="canonical"> dans index.html fait déjà le travail côté HTML
